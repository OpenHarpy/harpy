...
def install_packages(session, package_names: list) -> bool:
    ts = session.create_task_set().add_maps([MapTask(name='install-packages', fun=install_package_task, args=[], kwargs={'package_names': package_names})])
...
def uninstall_packages(session, package_names: list) -> bool:
    ts = session.create_task_set().add_maps([MapTask(name='uninstall-packages', fun=uninstall_packages_task, args=[], kwargs={'package_names': package_names})])
    result = ts.run(collect=True)
    if result[0] != 0:
        return False
    return True
...
def get_installed_packages(session) -> list:
    ts = session.create_task_set().add_maps([MapTask(name='get-installed-packages', fun=get_installed_packages_task, args=[], kwargs={})])
    result = ts.run(collect=True)
    return result[0]